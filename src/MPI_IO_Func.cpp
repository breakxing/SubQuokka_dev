...
void IO_Runner::_two_gate_mpi_read1_recv1(thread_IO_task &task,Gate * &g)
{
...
void IO_Runner::_mpi_one_gate_inner(thread_IO_task &task,Gate * &g)
{
    ...
    if(num_worker == 1)
    {
        ...
        else
        {
            ...
            g->run_one_qubit_mpi_io(task.buffer1,task.buffer2,0,0,1);
    ...
    g->run_one_qubit_mpi_io(*buffer1,*buffer2,member_th * half_round_chunk,member_th * half_round_chunk,half_round_chunk);
...
void IO_Runner::_thread_read2_recv2(thread_IO_task &task,Gate * &g,int num_worker,long long stride)
{
...
void IO_Runner::_thread_read1_recv3(thread_IO_task &task,Gate * &g,int num_worker)
{
    int member_th;
    for(int i = 0;i < 4;i++)
    {
...
void IO_Runner::_thread_no_exec_MPI(thread_IO_task &task,int round)
{
...
int IO_Runner::Get_Next_Undone_Buffer_index(vector<MPI_Request> &request,vector<bool>&used,int round,int start)
{
    int res = -1;
    int fl;
    int done_cnt = 0;
