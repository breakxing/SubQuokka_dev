...
thread_DIO_task::thread_DIO_task(int tid,int MPI_buffer_size) {
...
DIO_Runner::DIO_Runner() {
    ...
    for (int i = 0; i < env.num_thread; i++) {
        thread_tasks.push_back(thread_DIO_task(i,MPI_buffer_size));
        if(posix_memalign((void **)(&(thread_tasks[i].buffer1)), 4096, MPI_buffer_size * env.chunk_size));
        if(env.is_MPI)
        {
            if(posix_memalign((void **)(&(thread_tasks[i].buffer2)), 4096, MPI_buffer_size * env.chunk_size));
            if(posix_memalign((void **)(&(thread_tasks[i].buffer3)), 4096, MPI_buffer_size * env.chunk_size));
            if(posix_memalign((void **)(&(thread_tasks[i].buffer4)), 4096, MPI_buffer_size * env.chunk_size));
        }
    }
}
DIO_Runner::~DIO_Runner(){
    for (int i = 0; i < env.num_thread; i++) {
        delete [] thread_tasks[i].buffer1;
        if(env.is_MPI)
        {
            delete [] thread_tasks[i].buffer2;
            delete [] thread_tasks[i].buffer3;
            delete [] thread_tasks[i].buffer4;
        }
    }
}
void DIO_Runner::setFD(thread_DIO_task &task, Gate *&gate) {
...
void DIO_Runner::setFD_sub(thread_DIO_task &task) {
    task.fd_using.resize(0);
    task.fd_offset_using.resize(0);
...
void outer_loop_m3(thread_DIO_task &task, function<void()> &inner_loop, int m0, int m1, int m2) {
    long long half_off_m0 = env.qubit_offset[m0];
    long long half_off_m1 = env.qubit_offset[m1];
...
void outer_loop_m4up_helper(thread_DIO_task &task, function<void()> &inner_loop, vector<int> &m, int level){
    if (level == -1){
        inner_loop();
        for (auto &x : task.fd_offset_using) {
            x += env.chunk_size;
...
void outer_loop_m4up(thread_DIO_task &task, function<void()> &inner_loop, vector<int> m) {
...
void DIO_Runner::run(vector<Gate *> &circuit) {
#pragma omp parallel
    {
