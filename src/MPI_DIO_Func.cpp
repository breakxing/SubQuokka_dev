...
bool DIO_Runner::skip_read_write(Gate * & g,const int &idx)
{
    ...
    else if(g->name == "CPhase_Gate_DIO" && (idx != 1) && (!isChunk(g->targs[1]) && (isChunk(g->targs[0])) && (!isMpi(g->targs[1])))) return true;
...
void DIO_Runner::MPI_Swap(thread_DIO_task &task,Gate * &g)
{
    ...
    task.partner_using = {env.rank ^ mpi_targ_mask_1};
    if(isMpi(g->targs[0]))
    {
    ...
    else if(isFile(g->targs[0]))
    {
        ...
        else
        {
            ...
            int one_round_chunk = min((long long) MPI_buffer_size,(env.thread_state / env.chunk_state) >> 1);
            loop_stride = one_round_chunk * env.chunk_size;
            for(long long cur_offset = 0;cur_offset < loop_bound;cur_offset += loop_stride)
            {
    ...
    else
    {
        ...
        for(long long i = 0;i < env.thread_size;i+=env.chunk_size)
        {
            ...
            for(long long chunk_off = 0;chunk_off < env.chunk_state;chunk_off+=(one_time_copy_state << 1))
            {
                ...
                copy(task.buffer1 + startIdx0,task.buffer1 + startIdx0 + one_time_copy_state,task.buffer2 + startIdx1);
                ...
                if(st_tmp.size() * one_time_copy_state == (unsigned long int)per_state_transfer)
                {
                    ...
                    while(!st_tmp.empty())
                    {
                        ...
                        copy(task.buffer3 + buffer_size,task.buffer3 + buffer_size + one_time_copy_state,task.buffer1 + off);
                        buffer_size-=one_time_copy_state;
                    }
...
void DIO_Runner::MPI_gate_scheduler(thread_DIO_task &task,Gate * &g)
{
    ...
    else
    {
        if(isMpi(g->targs[0]))
        {
            ...
            complex<double>**buffer1_ptr = &task.buffer1;
            complex<double>**buffer2_ptr = &task.buffer2;
            complex<double>**buffer3_ptr = &task.buffer3;
            complex<double>**buffer4_ptr = &task.buffer4;
            ...
            for(long long cur_offset = 0;cur_offset < loop_bound;cur_offset += loop_stride)
            {
                ...
                g->run_mpi_u2_nonchunk_dio(*buffer1_ptr,*buffer2_ptr,*buffer3_ptr,*buffer4_ptr,per_chunk);
                for(int i = 0;i < per_chunk;i++)
                {
...
void DIO_Runner::_mpi_one_gate_inner(thread_DIO_task &task,Gate * &g)
{
    ...
    g->run_one_qubit_mpi_dio(*buffer1,*buffer2,0,0,one_round_chunk);
    for(int i = 0;i < one_round_chunk;i++)
    {
        if(pwrite(task.fd_using[0],&task.buffer1[i * env.chunk_state],env.chunk_size,task.fd_offset_using[0] + i * env.chunk_size)); 
    }
