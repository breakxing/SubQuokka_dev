...
for n in [N]:
    args = Args(total_qbit=n, file_qbit=file_seg, chunk_qbit=chunk_seg,mpi_qbit=mpiqubit,
                is_subcircuit=0,MPI_testing=1,MPI_buffe_size=16,
    ...
    if args.mpi_qbit == 0:
        ...
        # os.system(f"UCX_TLS=tcp mpirun -x UCX_NET_DEVICES={RDMA_TCP_CARD1},{RDMA_TCP_CARD2} -x LD_LIBRARY_PATH --bind-to none --hostfile ../hf --map-by ppr:1:node \"$(pwd)/../Quokka\" -i {ini_path} -c {cir_path}")
