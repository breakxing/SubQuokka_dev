...
for n in [20]:
    ...
    if args.mpi_qbit == 0:
        ...
        for i in range(8,16):
            os.system(f"scp -P 9037 paslab@140.112.90.37:~/SubQuokka/src/correctness/state/path{i - 8} ./state/path{i}")
        state_path_compare = [f"./state/path{i}" for i in range((1 << file_seg) << 1)]
