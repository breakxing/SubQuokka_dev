...
void findChunkbyMaxNode(int, Node *(&), set<int> &, const vector<string> &, const vector<string> &, vector<Node> &);
void findChunkbyMaxNodeSmall(int, Node *(&), set<int> &, int &);
void popGate(Node *(&), set<int> &, vector<string> &, vector<string> &);
void freeNodes(Node *(&));
//void copyList(const Node *(&), Node *(&));
...
void findChunkbyMaxNode(int chunkSize, Node *(&nList), set<int> &chunkSet, const vector<string> &gateVec, vector<Node> &dags) {
    Node *head = nList;
    int maxNodeSize = 0;
    chunkSet = {};
    set<string> names = {};
        ...
        while(nList->next) {
            ...
            if(tmpNodeSize <= chunkSize - maxNodeSize) {
                    ...
                    names.find(nList->name) == names.end())
                {
                    ...
                    /*
                    Node *nListTmp = head;
                    vector<Node*> tailsCopy (dags.size());
                    for(int i = 0; i < dags.size(); i++) {
                        tailsCopy[i] = &(dags[i]);
                    }
                    vector<string> gateVecCopy(gateVec);
                    vector<string> popGateVecCopy;
                    set<int> chunkSetCopy(chunkSet.begin(), chunkSet.end());
                    Node *nListCopy = new Node();                    
                    setDAG(gateVecCopy, tailsCopy, nListCopy);
                    popGate(nListCopy, tmpChunkSet, gateVecCopy, popGateVecCopy);
                    freeNodes(nListCopy);
                    nListCopy = new Node();
                    Node *head = nListCopy;
                    setDAG(gateVecCopy, tailsCopy, nListCopy);
                    findChunkbyMaxNodeSmall(chunkSize, nListCopy, chunkSetCopy, tmpDepNameSizeSecond);
                    tmpDepNameSize += tmpDepNameSizeSecond;
                    cout << "[Now Size]: " << tmpDepNameSize << endl;
                    nListCopy = head;
                    freeNodes(nListCopy);
                    */
                }
...
void findChunkbyMaxNodeSmall(int chunkSize, Node *(&nList), set<int> &chunkSet, int &tmpDepNameSize) {
    Node *head = nList;
    int maxNodeSize = 0;
    chunkSet = {};
    set<string> names = {};
...
int main(int argc, char *argv[]) {
    ...
    while(nList->next) {
        ...
        if(iter != 0)
            findChunkbyMaxNode(chunkSize, nList, chunkSet, gateVec, dags);
